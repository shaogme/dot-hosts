...
# 若本地存在有效的 dot-base / dot-exts 源码目录，自动覆盖 npins 避免开发测试时指向远端旧 commit
if [ -f "$REPO_DIR/dot-base/default.nix" ] && [ -z "${NPINS_OVERRIDE_dot_base:-}" ]; then
  export NPINS_OVERRIDE_dot_base="$REPO_DIR/dot-base"
fi
if [ -f "$REPO_DIR/dot-exts/default.nix" ] && [ -z "${NPINS_OVERRIDE_dot_exts:-}" ]; then
